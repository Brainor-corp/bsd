...
class StreamSMSHelper
{
    ...
    /*				(необязательный параметр)														  */
    /*------------------------------------------------------------------------------------------------*/
    function SendSms($server,$session,$sourceAddress,$destinationAddress,$data,$validity)
    {
        $href = $server.'Send/SendSms/';
        $src = 'sessionId='.$session.'&sourceAddress='.$sourceAddress.'&destinationAddress='.$destinationAddress.'&data='.$data.'&validity='.$validity;
        $result = $this -> PostConnect($src,$href);
        return json_decode($result,true);
    }
