...
<div>
    ...
    @if(!empty($documentData['ДатаИсполненияС']))
        <span>С: <i><b><u>{{ $documentData['ДатаИсполненияС'] ?? '' }}</u></b></i></span>
    @endif
    @if(!empty($documentData['ДатаИсполненияПо']))
        <span>До: <i><b><u>{{ $documentData['ДатаИсполненияПо'] ?? '' }}</u></b></i></span>
    @endif
    ...
    @endif
    <span style="">
        Примечания по грузу: {{ $documentData['Комментарий'] ?? '' }}
    </span><br><br>
    ...
    <span>Название компании отправителя или частное лицо (Ф.И.О.): <i><b><u>{{ $documentData['Грузоотправитель'] ?? '' }}</u></b></i></span><br>
    @if(!empty($documentData['ГрузоотправительИНН']))
        <span>ИНН : <i><b><u>{{ $documentData['ГрузоотправительИНН'] ?? '' }}</u></b></i></span><br>
    @endif
    @if(!empty($documentData['ГрузоотправительКПП']))
        <span>КПП : <i><b><u>{{ $documentData['ГрузоотправительКПП'] ?? '' }}</u></b></i></span><br>
    @endif
    <span>
        ...
        <i>
            <b>
                <u>
                    {{ $documentData['ГрузоотправительТелефон'] ?? '' }}
                    {{ !empty($documentData['КонтактноеЛицоГрузоотправителя']) ? "(" . $documentData['КонтактноеЛицоГрузоотправителя'] . ")" : '' }}
                </u>
            </b>
        </i>
    </span><br>
    <span>Полный адрес (в случае необходимости забора груза "от двери" отправителя): <i><b><u>{{ $documentData['АдресЗабора'] ?? '-' }}</u></b></i></span><br>
    <span>Нужен ли забор от адреса отправителя: <i><b><u>{{ !empty($documentData['АдресЗабора']) ? 'Да' : 'Нет' }}</u></b></i></span><br>
    <span>Режим работы склада: <i><b><u>{{ !empty($documentData['РежимРаботыСклада']) ? $documentData['РежимРаботыСклада'] : 'с 9:00 до 18:00' }}</u></b></i></span><br>
    <span>Дополнительная информация: <i><b><u>{{ $documentData['ГрузоотправительДопИнформация'] ?? '' }}</u></b></i></span><br>
    ...
    @if(!empty($documentData['ГрузополучательИНН']))
        <span>ИНН : <i><b><u>{{ $documentData['ГрузополучательИНН'] ?? '' }}</u></b></i></span><br>
    @endif
    @if(!empty($documentData['ГрузополучательКПП']))
        <span>КПП : <i><b><u>{{ $documentData['ГрузополучательКПП'] ?? '' }}</u></b></i></span><br>
    @endif
    <span>
        Телефон (контактное лицо):
        <i>
            <b>
                <u>
                    {{ $documentData['ГрузополучательТелефон'] ?? '' }}
                    {{ !empty($documentData['КонтактноеЛицоГрузополучателя']) ? "(" . $documentData['КонтактноеЛицоГрузополучателя'] . ")" : '' }}
                </u>
            </b>
        </i>
    </span><br>
    ...
    <span>Нужна ли доставка до адреса получателя: <i><b><u>{{ !empty($documentData['АдресДоставки']) ? 'Да' : 'Нет' }}</u></b></i></span><br>
    <span>Дополнительная информация: <i><b><u>{{ $documentData['ГрузополучательДопИнформация'] ?? '' }}</u></b></i></span><br>
    ...
    @if(!empty($documentData['ПлательщикИНН']))
        <span>ИНН : <i><b><u>{{ $documentData['ПлательщикИНН'] ?? '' }}</u></b></i></span><br>
    @endif
    @if(!empty($documentData['ПлательщикКПП']))
        <span>КПП : <i><b><u>{{ $documentData['ПлательщикКПП'] ?? '' }}</u></b></i></span><br>
    @endif
    <span>
        Телефон (контактное лицо):
        <i>
            <b>
                <u>
                    ...
                    {{ !empty($documentData['КонтактноеЛицоПлательщика']) ? "(" . $documentData['КонтактноеЛицоПлательщика'] . ")" : '' }}
                </u>
            </b>
        </i>
    </span><br>
    ...
    <span>Дополнительная информация: <i><b><u>{{ $documentData['ПлательщикДопИнформация'] ?? '' }}</u></b></i></span><br>
