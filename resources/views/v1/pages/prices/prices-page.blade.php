            ...
            <div class="row">
                <div class="col-12">
                        ...
                        <div class="col-12">
                            <form action="{{ route('pricesPage') }}" method="get">
                                ...
                                <div class="form-check form-check-inline">
                                    <div class="form-group">
                                        ...
                                        <select id="dest_city" class="form-control point-select" name="dest_city" placeholder="Выберите город" required>
                                            <option value=""></option>
                                            @foreach($shipCities as $shipCity)
                                                <option value="{{ $shipCity->id }}" @if($destCityId == $shipCity->id) selected @endif>{{ $shipCity->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-check form-check-inline">
                                    <div class="form-group">
                                        <label> </label>
                                        <button type="submit" class="btn margin-item btn-danger form-control">Показать</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            @if(isset($route))
                                <h5>Тарифы на грузоперевозки маршрута {{ $route->name }}</h5>
                                    ...
                                    <table class="table table-bordered text-center">
                                        ...
                                        <tr>
                                            @foreach($route->route_tariffs->where('rate.slug', 'ves') as $routeTariff)
                                                <th>До {{ $routeTariff->threshold->value }}кг</th>
                                            @endforeach
