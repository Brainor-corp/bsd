...
<div class="form-item row align-items-center">
    ...
    <div class="col calc__inpgrp">
        <div class="input-group">
            <input type="text" pattern="\d{4}" maxlength="4" class="form-control text-center form-item req" name="recipient_passport_series" value="{{ $order->recipient_passport_series ?? '' }}" placeholder="Серия" {{ isset($disabled) ? 'disabled' : 'required' }} />
            <input type="text" pattern="\d{6}" maxlength="6" class="form-control text-center form-item req" name="recipient_passport_number" value="{{ $order->recipient_passport_number ?? '' }}" placeholder="Номер" {{ isset($disabled) ? 'disabled' : 'required' }} />
        </div>
    </div>
</div>
<div class="form-item row align-items-center">
